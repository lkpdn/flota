...
impl YastFlash {
    pub fn new(img_path_on_host: &Path,
               ...
               script: &str)
               -> Result<YastFlash> {
        let yast_img_filename = img_path_on_host.file_name().unwrap();
        let yast_script_filename = script_path_on_guest.file_name().unwrap();
        let tmp = Path::new("/tmp");
        let tmp_mount = tmp.join(format!("{}.d", yast_img_filename.to_str().unwrap()));
        let tmp_yast_script_path = tmp.join(yast_script_filename);
        ...
        let mut f = try!(File::create(&tmp_yast_script_path));
        try!(f.write_all(script.to_string().into_bytes().as_slice()));
        try!(fs::copy(&tmp_yast_script_path,
                      format!("{}/{}",
                              tmp_mount.to_str().unwrap(),
                              yast_script_filename.to_str().unwrap())
                          .as_str()));
        if !Command::new("sudo")
