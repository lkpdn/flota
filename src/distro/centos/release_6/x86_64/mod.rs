...
lazy_static! {
    pub static ref ISO: Url = Url::parse(
        "http://ftp.riken.jp/Linux/centos/6.8/isos/x86_64/CentOS-6.8-x86_64-minimal.iso").unwrap();
    pub static ref ISO_MD5SUM: Url = Url::parse(
        "http://ftp.riken.jp/Linux/centos/6.8/isos/x86_64/md5sum.txt").unwrap();
    pub static ref VMLINUZ: Url = Url::parse(
        "http://ftp.riken.jp/Linux/centos/6.8/os/x86_64/isolinux/vmlinuz").unwrap();
    pub static ref INITRD: Url = Url::parse(
        "http://ftp.riken.jp/Linux/centos/6.8/os/x86_64/isolinux/initrd.img").unwrap();
}
const IDENT: &'static str = "centos6-x86_64";
        ...
        unsafe {
            match virDomainLookupByName(conn.raw(), rawCharPtr!(dom_name)) {
                ...
                _ => {
                    ...
                    x_os.tag(xE!("cmdline"))
                        .text(format!(
                         "fbcon=map:99 text console=ttyS0 ks=hd:fd0:/{} xhci_hcd.quirks=262144",
                         ks_floppy.script_path_on_guest.to_str().unwrap()
                       )
                            .into());
                    ...
                    // kickstart floppy
                    x_dev.tag(xE!("disk", type => "file", device =>  "floppy"))
                        .tag_stay(xE!("source",
                        file => ks_floppy.img_path_on_host.to_str().unwrap()))
                        .tag_stay(xE!("target", dev => "fda"))
                        .tag_stay(xE!("read_only"));
                    // default network interface
                    x_dev.tag(xE!("interface", type => "network"))
                        .tag(xE!("source", network => network.name()));
                    // console
                    // if libvirt version >=1.3.3
